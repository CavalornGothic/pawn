...
//mysql_connect("sdc2.netshoot.pl", "baza_19496", "baza_19496", "sfyvqfidaj");
	...
	handle = db_open(DATABASE);
	if(handle)
	{
	    print("[System] Polaczono z baza danych!");
	}
	SendRconCommand("mapname FortCarson[PL][RPG]");
	SendRconCommand("gamemodetext FDRC[PL][RPG]");
	SendRconCommand("language PL;Polish;Polski");
	SendRconCommand("hostname FDRC Server - Klasyczna rozgrywka jak w pocz¹tkach SAMP!");
	gstatus = 1;
	thorus_loadVeh();
	thorus_loadOrg();
	thorus_loadBuilding();
		...
		if(biz[i][bizid] != 0)
		{
			if(IsPlayerInRangeOfPoint(playerid, 1.0, biz[i][ex], biz[i][ey], biz[i][ez]))
			{
				...
				if(biz[i][idown] != 0)
				{
					...
					result = db_query(handle, sq);
					db_get_field(result, 0, nick_gracza, 24);
...
public thorus_loadBusiness()
{
    ...
    result = db_query(handle, "select * from `biz`");
    
	if(db_num_rows(result) < 1)
	    return print("[System] Nie ma danych do wczytania !");
	    
	ilebiz = 0;
	new ile2 = db_num_rows(result);
	while(ile2 != 0)
	{
	    ilebiz++;
	    biz[ilebiz][bizid] = db_get_field_int(result, 0);
	    db_get_field(result, 1, biz[ilebiz][nazwa], 32);
	    biz[ilebiz][typ] = db_get_field_int(result, 2);
	    biz[ilebiz][ex] = db_get_field_float(result, 3);
	    biz[ilebiz][ey] = db_get_field_float(result, 4);
	    biz[ilebiz][ez] = db_get_field_float(result, 5);
	    biz[ilebiz][eint] = db_get_field_int(result, 6);
	    biz[ilebiz][ex2] = db_get_field_float(result, 7);
	    biz[ilebiz][ey2] = db_get_field_float(result, 8);
	    biz[ilebiz][ez2] = db_get_field_float(result, 9);
	    biz[ilebiz][eint2] = db_get_field_int(result, 10);
	    biz[ilebiz][konto] = db_get_field_int(result, 11);
	    biz[ilebiz][produkty] = db_get_field_int(result, 12);
	    biz[ilebiz][zamkniety] = db_get_field_int(result, 13);
		biz[ilebiz][pickupid3] = db_get_field_int(result, 14);
		biz[ilebiz][idown] = db_get_field_int(result, 15);
		biz[ilebiz][cena] = db_get_field_int(result, 16);
	...
	printf("[System] wczytano %d biznesow", ilebiz);
	db_free_result(result);
	/*
	// Wczytywanie danych z bazy MySQL
    new query[256];
...
public thorus_saveBusiness(uid)
{
	...
	biz[uid][cena],
	biz[uid][bizid]);
	db_query(handle, datasave);
	//mysql_query(datasave);
	return 1;
}

stock insertid(table[])
{
	new id_insert, zapytanie[64];
	format(zapytanie, sizeof zapytanie, "select max(id) from `%s`", table);
	result = db_query(handle, zapytanie);
	id_insert = db_get_field_int(result, 0);
	db_free_result(result);
	return id_insert;
}
...
public thorus_createBusiness(playerid, b_name[24], b_typ)
{
	...
	py,
	pz);
	db_query(handle, insertdata);
    new uid = ilebiz;
    new iddata = insertid("biz");
...
public thorus_loadBuilding()
{
    ...
    result = db_query(handle, "select * from `bud`");
	...
	{
	    ilebud++;
	    bud[ilebud][bid] = db_get_field_int(result, 0);
	    db_get_field(result, 1, bud[ilebud][nazwa], 32);
	    bud[ilebud][ex] = db_get_field_float(result, 2);
	    bud[ilebud][ey] = db_get_field_float(result, 3);
	    bud[ilebud][ez] = db_get_field_float(result, 4);
	    bud[ilebud][ea] = db_get_field_float(result, 5);
	    bud[ilebud][eint] = db_get_field_int(result, 6);
	    bud[ilebud][ex2] = db_get_field_float(result, 7);
	    bud[ilebud][ey2] = db_get_field_float(result, 8);
	    bud[ilebud][ez2] = db_get_field_float(result, 9);
	    bud[ilebud][ea2] = db_get_field_float(result, 10);
	    bud[ilebud][eint2] = db_get_field_int(result, 11);
	    bud[ilebud][pickupid2] = db_get_field_int(result, 12);
	    bud[ilebud][index_t] = ilebud;
	    bud[ilebud][pickupid2] = CreateDynamicPickup(1239, 1, bud[ilebud][ex], bud[ilebud][ey], bud[ilebud][ez]);
	    //printf("id: %d | %s", bud[ilebud][bid], bud[ilebud][nazwa]);
	    db_next_row(result);
	    ile2--;
	}
	db_free_result(result);
...
public thorus_buildingCreate(playerid, b_name[24])
{
	...
	new idbud = ilebud++;
	new ins_id = db_insert_id("bud", "nazwa", b_name);
	ins_id++;
...
public thorus_loadOrg()
{
	...
	result = db_query(handle, "select * from `org`");

	if(db_num_rows(result) < 1)
	    return print("[System] Nie ma danych do wczytania !");

	new ile2 = db_next_row(result);
	while(ile2 != 0)
	{
	    org[ileorg][oid] = db_get_field_int(result, 0);
        db_get_field(result, 1, org[ileorg][onazwa], 32);
        org[ileorg][typ] = db_get_field_int(result, 2);
        org[ileorg][ox] = db_get_field_float(result, 3);
        org[ileorg][oy] = db_get_field_float(result, 4);
        org[ileorg][oz] = db_get_field_float(result, 5);
        org[ileorg][oa] = db_get_field_float(result, 6);
        org[ileorg][r_skin] = db_get_field_int(result, 7);
        db_get_field(result, 8, org[ileorg][r_nazwa], 32);
        org[ileorg][r_skin2] = db_get_field_int(result, 9);
        db_get_field(result, 10, org[ileorg][r_nazwa2], 32);
        org[ileorg][r_skin3] = db_get_field_int(result, 11);
        db_get_field(result, 12, org[ileorg][r_nazwa3], 32);
        org[ileorg][r_skin4] = db_get_field_int(result, 13);
        db_get_field(result, 14, org[ileorg][r_nazwa4], 32);
        org[ileorg][r_skin5] = db_get_field_int(result, 15);
        db_get_field(result, 16, org[ileorg][r_nazwa5], 32);
        org[ileorg][r_skin6] = db_get_field_int(result, 17);
        db_get_field(result, 18, org[ileorg][r_nazwa6], 32);
        org[ileorg][r_skin7] = db_get_field_int(result, 19);
        db_get_field(result, 20, org[ileorg][r_nazwa7], 32);
        org[ileorg][r_skin8] = db_get_field_int(result, 21);
        db_get_field(result, 22, org[ileorg][r_nazwa8], 32);
        org[ileorg][r_skin9] = db_get_field_int(result, 23);
        db_get_field(result, 24, org[ileorg][r_nazwa9], 32);
        org[ileorg][r_skin10] = db_get_field_int(result, 25);
        db_get_field(result, 26, org[ileorg][r_nazwa10], 32);
        ileorg++;
        ile2--;
        db_next_row(result);
	}
...
public thorus_OnPlayerLogin(playerid, password[])
{
	...
	result = db_query(handle, mq);
	db_get_field(result, 0, p[playerid][haslo], 24);
	db_free_result(result);
	if(!strcmp(p[playerid][haslo], password, true))
	{
		format(mq, sizeof mq, "select * from `players` where `login`='%s'", PlayerName(playerid));
		result = db_query(handle, mq);
		new login2[24], haslo2[24];
		p[playerid][id] = db_get_field_int(result, 0);
		db_get_field(result, 1, login2, 24);
		db_get_field(result, 2, haslo2, 24);
		p[playerid][kasa] = db_get_field_int(result, 3);
		p[playerid][level] = db_get_field_int(result, 4);
		p[playerid][exp] = db_get_field_int(result, 5);
		p[playerid][wexp] = db_get_field_int(result, 6);
        p[playerid][skin] = db_get_field_int(result, 7);
        new Float:pxx = db_get_field_float(result, 8);
        new Float:pxy = db_get_field_float(result, 9);
        new Float:pxz = db_get_field_float(result, 10);
        new Float:pxa = db_get_field_float(result, 11);
        p[playerid][prawko] = db_get_field_int(result, 12);
        p[playerid][pilotaz] = db_get_field_int(result, 13);
        p[playerid][motor] = db_get_field_int(result, 14);
        p[playerid][lickabron] = db_get_field_int(result, 15);
        p[playerid][admin] = db_get_field_int(result, 16);
        p[playerid][vip] = db_get_field_int(result, 17);
        p[playerid][praca] = db_get_field_int(result, 18);
        p[playerid][idf] = db_get_field_int(result, 19);
        p[playerid][ranga] = db_get_field_int(result, 20);
        p[playerid][slot] = db_get_field_int(result, 21);
		p[playerid][slot1] = db_get_field_int(result, 22);
		p[playerid][slot2] = db_get_field_int(result, 23);
		p[playerid][ammo2] = db_get_field_int(result, 24);
		p[playerid][slot3] = db_get_field_int(result, 25);
		p[playerid][ammo3] = db_get_field_int(result, 26);
		p[playerid][slot4] = db_get_field_int(result, 27);
		p[playerid][ammo4] = db_get_field_int(result, 28);
		p[playerid][slot5] = db_get_field_int(result, 29);
		p[playerid][ammo5] = db_get_field_int(result, 30);
		p[playerid][slot6] = db_get_field_int(result, 31);
		p[playerid][ammo6] = db_get_field_int(result, 32);
		p[playerid][slot9] = db_get_field_int(result, 33);
		p[playerid][ammo9] = db_get_field_int(result, 34);
		p[playerid][slot10] = db_get_field_int(result, 35);
		db_free_result(result);
		printf("%d", p[playerid][admin]);
	    /*
	    new data[256];
	    format(bq, sizeof bq, "select * from `players` where `login`='%s'  ", PlayerName(playerid));
	   	result = db_query(handle, bq);
		mysql_store_result();
		...
		p[playerid][ammo9],
		p[playerid][slot10]); */
		SetPlayerPos(playerid, pxx, pxy, pxz);
		SetPlayerFacingAngle(playerid, pxa);
...
public thorus_ShowStatsPlayer(playerid)
{
	...
	format(statys, sizeof statys, "Prawojazdy kat. B: %s - Prawojazdy kat. A: %s - Pozwolenie na broñ: %s - Mo¿liwoœæ pilota¿u: %s - Poziom admina: %d", praw, mot, licka, pil, p[playerid][admin]);
	SendClientMessage(playerid, zolty, statys);
	return 1;
}
...
public thorus_loadVeh()
{
	...
	result = db_query(handle, "SELECT * FROM `veh`");
	//mysql_store_result();
	vid3++;
	//new fetch[128];
	...
	while(rows != 0)
	{
	    new uid = db_get_field_int(result, 0);
	    v[uid][model] = db_get_field_int(result, 1);
	    v[uid][vx] = db_get_field_float(result, 2);
	    v[uid][vy] = db_get_field_float(result, 3);
	    v[uid][vz] = db_get_field_float(result, 4);
	    v[uid][va] = db_get_field_float(result, 5);
	    v[uid][kolor] = db_get_field_int(result, 6);
	    v[uid][kolor2] = db_get_field_int(result, 7);
	    v[uid][typ] = db_get_field_int(result, 8);
	    v[uid][fid] = db_get_field_int(result, 9);
	    v[uid][cena] = db_get_field_int(result, 10);
	    v[uid][ido] = db_get_field_int(result, 11);
	    v[uid][vid] = uid;
		if(v[uid][model] != 0)
		{
...
db_insert_id(table[], column[], name[])
{
	new dii[128], id_dii;
	format(dii, sizeof dii, "select `id` from `%s` where `%s`='%s'", table, column, name);
	result = db_query(handle, dii);
	if(db_num_rows(result) < 1 || db_num_rows(result) > 1)
	{
	    db_free_result(result);
	    return 0;
	}
	...
	{
	    id_dii = db_get_field_int(result, 0);
	    db_free_result(result);
	    return id_dii;
	}
	db_free_result(result);
...
public thorus_orgAdd(o_name[24], o_typ, Float:o_x, Float:o_y, Float:o_z, Float:o_a)
{
	...
	db_query(handle, insert);
	new uid = ileorg+1;
	new idorg = db_insert_id("org", "nazwa", o_name);
	org[uid][oid] = idorg;
...
CMD:komendy(playerid, params[])
{
	...
	SendClientMessage(playerid, zolty, "/statystyki - /zmienhaslo - /me - /pw - /do - /b - /go - /report - /dajkase - /orgcmd - /drzwi - /sprzedajpojazd - /kup");
...
CMD:acaradd(playerid, params[])
{
	...
	db_query(handle, addcar);
	result = db_query(handle, "select max(id) from `veh`");
	uid = db_get_field_int(result, 0);
	db_free_result(result);
	v[uid][vid] = uid;
	v[uid][model] = 481;
    v[uid][vx] = px;
