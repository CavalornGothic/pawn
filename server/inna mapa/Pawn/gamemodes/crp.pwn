...
public OnPlayerRequestClass(playerid, classid)
{
	...
	if (hFile)
	{
		ShowPlayerDialog(playerid, logowanie, DIALOG_STYLE_PASSWORD, "Logowanie", "Witaj!\nJesteœ ju¿ naszym graczem.\nPodaj has³o do konta.", "Zaloguj", "WyjdŸ");
		fclose(hFile);
	}
	else
	{
		ShowPlayerDialog(playerid, rejestracja, DIALOG_STYLE_PASSWORD, "Rejestracja", "Witaj!\nAby móc zagraæ na serwerze najpierw musisz siê zarejestrowaæ.\nRejestruj¹c siê akceptujesz nasze zasady gry dostêpne na forum serwera.\nPodaj has³o do konta:", "Zarejestruj", "WyjdŸ");
	}
   	return 0;
}
public OnPlayerRequestSpawn(playerid)
{
	...
	else
	{
	    ...
	    {
	        KickPlayer(playerid,"System","Zbyt du¿a ilosæ prób spawnu bez zalogowania.");
			return 1;
	    }
	...
	if(IsPlayerConnected(killerid))
	{
	    if(killerid != playerid)
	    {
	     	if(AdminDuty[playerid])
		    {
		        ...
		        {
					KickPlayer(killerid,"System","Próba zabicia(lub zabicie) administratora na s³u¿bie.");
...
public OnPlayerText(playerid, text[])
{
 	...
 	if((strcmp("(", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("(")))
	{
	    if(text[1] != 0)
	    {
		    format(string, sizeof(string), "(( [Lokalne OOC:] %s mówi: %s ))", PlayerName(playerid),text[1]);
	...
	{
		format(string, sizeof(string), "[911:] %s(ID:%d) mówi: %s",GetPlayerNameEx(playerid),playerid,text);
		SendFactionTypeMessage(1, COLOR_LSPD, string);
		SendClientMessage(playerid,COLOR_WHITE,"Operator: Proszê mówiæ, rozmowa zostanie nagrana.");
		Mobile[playerid] = 255;
	...
	if(Mobile[playerid] != 255)
	{
		...
		if(IsPlayerConnected(Mobile[playerid]))
		{
		    if(Mobile[Mobile[playerid]] == playerid)
		    {
				...
				{
					format(string, sizeof(string), "[Mówca] %s mówi: %s", GetPlayerNameEx(playerid), text);
...
//========================================[HELP & INFORMATION COMMANDS]===================================================
 	...
 	if(strcmp(cmd,"/sprzedajbron",true)==0)
    {
        if(IsPlayerConnected(playerid))
	    {
		    if (PlayerInfo[playerid][pJob] != 1)
			{
			    SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[ERROR:] Nie jesteœ dilerem broni.");
			    return 1;
			}
	...
	if(strcmp(cmd,"/materialy",true)==0)
    {
        if(IsPlayerConnected(playerid))
	    {
		    if (PlayerInfo[playerid][pJob] != 1)
			{
			    SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[ERROR:] Nie jesteœ dilerem dragów!");
			    return 1;
			}
			new x_nr[256];
			x_nr = strtok(cmdtext, idx);
			if(!strlen(x_nr)) {
				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[USAGE:] /materialy <czynnoœæ>");
			...
			if(strcmp(x_nr,"kup",true) == 0)
			{
			    if(PlayerToPoint(3.0,playerid,GunJob[BuyPackagesX],GunJob[BuyPackagesY],GunJob[BuyPackagesZ]))
			    {
			        ...
			        {
			            SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Nie mo¿esz kupiæ wiêcej paczek broni.");
				        return 1;
			        }
			        tmp = strtok(cmdtext, idx);
			        if(!strlen(tmp)) {
					...
					if(GetPlayerCash(playerid) > price)
					{
					    format(string, sizeof(string), "[INFO:] Masz %d paczek materia³ów - Koszt: %d$.", moneys, price);
					    SendClientMessage(playerid, COLOR_WHITE, string);
					    GivePlayerCash(playerid, - price);
					...
					{
					    SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Masz za ma³o pieniêdzy.");
					}
			    }
			    else
			    {
	...
	if(strcmp(cmd, "/proba", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
	    {
			...
			if(succeed == 1)
			{
				format(string, sizeof(string), "[ATTEMPT:] %s spróbowa³ %s z sukcesem.", GetPlayerNameEx(playerid), result);
				ProxDetector(10.0, playerid, string, COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE);
			}
			else if(succeed == 2)
			{
				format(string, sizeof(string), "[ATTEMPT:] %s spróbowa³ %s z pora¿k¹.", GetPlayerNameEx(playerid), result);
				ProxDetector(10.0, playerid, string, COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE,COLOR_LIGHTBLUE);
			}
		}
		return 1;
 	...
 	if(strcmp(cmd, "/sms", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			...
			if(!strlen(tmp))
			{
				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[USAGE:] /sms <nr telefonu> <wiadomoœæ>");
				return 1;
			}
					...
					if(PlayerInfo[i][pPhoneNumber] == phonenumb && phonenumb != 0)
					{
						...
						if(IsPlayerConnected(giveplayerid))
						{
						    if(giveplayerid != INVALID_PLAYER_ID)
						    {
								...
								format(string, sizeof(string), "[SMS:] Od: %s (%d), Treœæ: %s", GetPlayerNameEx(playerid),PlayerInfo[playerid][pPhoneNumber],result);
								SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[INFO:] Wys³ano.");
								SendClientMessage(giveplayerid, COLOR_LIGHTGREEN, string);
								format(string, sizeof(string), "[SMS:] Do: %s (%d), Treœæ: %s", GetPlayerNameEx(giveplayerid),PlayerInfo[giveplayerid][pPhoneNumber],result);
								SendClientMessage(playerid,  COLOR_LIGHTGREEN, string);
								PhoneAnimation(playerid);
								SMSLog(string);
			...
			SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Wiadomoœæ nie wys³ana.");
		}
		return 1;
	}
	...
	if(PlayerInfo[playerid][pPhoneNumber] != 0)
	{
		...
		{
		    SendClientMessage(playerid,  COLOR_LIGHTYELLOW2, "[TELEFON:] Twój numer telefonu nie bêdzie pokazywany publicznie.");
		    PlayerInfo[playerid][pListNumber] = 0;
		}
		else
		{
			SendClientMessage(playerid,  COLOR_LIGHTYELLOW2, "[TELEFON:] Twój numer telefonu bêdzie pokazywany publicznie.");
		 	PlayerInfo[playerid][pListNumber] = 1;
		}
	}
	else
 	...
 	if(strcmp(cmd, "/zadzwon", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			...
			if(phonenumb == 911)
			{
				SendClientMessage(playerid, COLOR_WHITE, "Operator: Dzieñ dobry, tutaj komisariat FCPD, w czym mogê pomóc?.");
				Mobile[playerid] = 911;
				return 1;
			}
						...
						if(IsPlayerConnected(giveplayerid))
						{
						    if(giveplayerid != INVALID_PLAYER_ID)
						    {
								...
								if (Mobile[giveplayerid] == 255)
								{
									...
									SendClientMessage(giveplayerid, COLOR_LIGHTGREEN, string);
									PlayerActionMessage(giveplayerid,15.0,"telefon zaczyna dzwoniæ.");
				    ...
				    if(Businesses[i][BizType] == 3) //24-7
			    	{
		        		if(Businesses[i][Products] != 0)
		        		{
							...
							if(!strlen(x_info)) {
								...
								SendClientMessage(playerid, COLOR_RED, "(Podaj nazwê, bez spacji, bez polskich znaków)");
								SendClientMessage(playerid, COLOR_WHITE, "* Baseball - Cena: 200$.");
								SendClientMessage(playerid, COLOR_WHITE, "* £opata - Cena: 100$.");
								SendClientMessage(playerid, COLOR_WHITE, "* Pi³a ³añcuchowa - Cena: 5000$.");
				...
				if(GetPlayerVirtualWorld(playerid) == i)
	   			{
				    if(Businesses[i][BizType] == 4) //Ammunation
			    	{
		        		if(Businesses[i][Products] != 0)
		        		{
							...
							if(PlayerInfo[playerid][pWepLic])
							{
								if(strcmp(x_info,"deagle",true) == 0)
								{
									if(GetPlayerCash(playerid) >= 3500)
									{
									    ...
									    SaveBusinesses();
									    PlayerActionMessage(playerid,15.0,"kupuje broñ.");
									    return 1;
							...
							{
								SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Nie masz licencji na broñ, mo¿esz kupiæ:");
								SendClientMessage(playerid, COLOR_RED, "(Wpisz nazwe, bez spacji)");
							...
							else if(strcmp(x_info,"bodyarmour",true) == 0)
							{
								if(GetPlayerCash(playerid) >= 1500)
								{
								    ...
								    {
								    	SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Twój wskaŸnik kamizelki jest pe³en!");
								    	return 1;
								    }
								}
							}
					...
					if(GetPlayerVirtualWorld(playerid) == i)
					{
			    		if(Businesses[i][BizType] == 7)
			    		{
							...
							if(!strlen(x_info)) {
								...
								SendClientMessage(playerid, COLOR_WHITE, "[ITEM:] Wódka - Cena: 10$.");
				        	...
				        	if(Businesses[i][Products] != 0)
				        	{
								...
								if(strcmp(x_info,"piwo",true) == 0)
								{
                              		...
                              		if(HP < 100)
                        		    {
							  		...
							  		PlayerActionMessage(playerid,15.0,"kupuje napój i go wypija.");
							  		SaveBusinesses();
							  		return 1;
									}
									else
	    ...
	    if(IsPlayerConnected(playerid))
		{
			...
			if(PlayerInfo[playerid][pHouseKey] != 255 && strcmp(playername, Houses[PlayerInfo[playerid][pHouseKey]][Owner], true) == 0)
			{
				...
				if(PlayerToPoint(1.0,playerid,Houses[house][EnterX],Houses[house][EnterY],Houses[house][EnterZ]))
				{
					...
					PlayerInfo[playerid][pHouseKey] = 255;
					OnPlayerDataSave(playerid);
       				PlayerActionMessage(playerid,15.0,"oddaje klucze agentowi nieruchomoœci.");
					SaveHouses();
					return 1;
				}
				else
				{
				    SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[ERROR:] To musi byæ Twój dom!");
				}
			}
			else
			{
	        ...
	        for(new i = 0; i < sizeof(Houses); i++)
			{
				if (PlayerToPoint(3.0, playerid,Houses[i][EnterX], Houses[i][EnterY], Houses[i][EnterZ]) || PlayerToPoint(3.0, playerid,Houses[i][ExitX], Houses[i][ExitY], Houses[i][ExitZ]))
				{
					if(PlayerInfo[playerid][pHouseKey] == i)
					{
						...
						{
							Houses[i][Locked] = 1;
							SendClientMessage(playerid, COLOR_LIGHTYELLOW2,"[INFO:] Drzwi zamkniête.");
 	...
 	if(strcmp(cmd, "/wynajem", true) == 0)
	{
			...
			for(new h = 0; h < sizeof(Houses); h++)
			{
				if(PlayerToPoint(2.0, playerid, Houses[h][EnterX], Houses[h][EnterY], Houses[h][EnterZ]) && Houses[h][Owned] == 1 &&  Houses[h][Rentable] == 1)
				{
					...
					if(GetPlayerCash(playerid) >= Houses[h][RentCost])
					{
						...
						SendClientMessage(playerid, COLOR_GREEN, "[INFO:] Dom wynajêty.");
						OnPlayerDataSave(playerid);
						return 1;
					}
					else
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[INFO:] Nie masz tyle!");
						return 1;
					}
				}
			}
 	...
 	if(strcmp(cmd, "/wynajemdom", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new bouse = PlayerInfo[playerid][pHouseKey];
			...
			if (bouse != 255 && strcmp(playername, Houses[PlayerInfo[playerid][pHouseKey]][Owner], true) == 0)
			{
				...
				if(!strlen(tmp))
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[USAGE:] /wynajemdom <cena>");
					return 1;
				}
				if(strval(tmp) < 1 || strval(tmp) > 99999)
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Cena: 1-99999$.");
					return 1;
				}
				Houses[bouse][RentCost] = strval(tmp);
				SaveHouses();
				format(string, sizeof(string), "[INFO:] Dodano oferte za: %d$", Houses[bouse][RentCost]);
				SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
				return 1;
			}
			else
 	...
 	if(strcmp(cmd, "/wynajemstatus", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new playername[MAX_PLAYER_NAME];
			...
			if (PlayerInfo[playerid][pHouseKey] != 255 && strcmp(playername, Houses[PlayerInfo[playerid][pHouseKey]][Owner], true) == 0)
			{
				...
				{
    				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[INFO:] Wynajem w³¹czono.");
				    Houses[PlayerInfo[playerid][pHouseKey]][Rentable] = 1;
				    SaveHouses();
				}
				else if(Houses[PlayerInfo[playerid][pHouseKey]][Rentable] == 1)
				{
    				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[INFO:] Wynajem wy³¹czono.");
			...
			{
				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "To nie Twój dom!");
				return 1;
			}
		}
		return 1;
	}
	if(strcmp(cmd, "/kupdom", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
	    {
			new Float:oldposx, Float:oldposy, Float:oldposz;
			...
			for(new h = 0; h < sizeof(Houses); h++)
			{
				if(PlayerToPoint(2.0, playerid, Houses[h][EnterX], Houses[h][EnterY], Houses[h][EnterZ]) && Houses[h][Owned] == 0)
				{
				    ...
				    {
				        SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Ten dom nie ma ceny, skontaktuj siê z administratorem.");
						return 1;
				    }
					if(PlayerInfo[playerid][pHouseKey] != 255 && strcmp(playername, Houses[PlayerInfo[playerid][pHouseKey]][Owner], true) == 0)
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Mo¿esz posiadaæ tlko jeden dom.");
						return 1;
					}
					if(GetPlayerCash(playerid) > Houses[h][HousePrice])
					{
						...
						SetPlayerVirtualWorld(playerid,h);
						SetPlayerPos(playerid,Houses[h][ExitX],Houses[h][ExitY],Houses[h][ExitZ]);
						SendClientMessage(playerid, COLOR_WHITE, "[INFO:] Kupiono dom!");
	       				PlayerActionMessage(playerid,15.0,"odbiera klucz od agenta nieruchomoœci do swojego nowego domu.");
 	...
 	if(strcmp(cmd, "/biznescenawejscia", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new bouse = PlayerInfo[playerid][pBizKey];
			...
			if (bouse != 255 && strcmp(playername, Businesses[PlayerInfo[playerid][pBizKey]][Owner], true) == 0)
			{
				...
				if(!strlen(tmp))
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[USAGE:] /biznescenawejscia <cena>");
				}
				...
				format(string, sizeof(string), "[INFO:] Wejœcie do biznesu kosztuje: %d$.", Businesses[bouse][EntranceCost]);
				SendClientMessage(playerid, COLOR_WHITE, string);
				SaveBusinesses();
			}
			else
 	...
 	if(strcmp(cmd, "/biznesnazwa", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new bouse = PlayerInfo[playerid][pBizKey];
			...
			if (bouse != 255 && strcmp(playername, Businesses[PlayerInfo[playerid][pBizKey]][Owner], true) == 0)
			{
				...
				if(!strlen(result))
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[USAGE:] /biznesnazwa <nazwa>");
				}
				if(PlayerToPoint(1.0,playerid,Businesses[bouse][EnterX],Businesses[bouse][EnterY],Businesses[bouse][EnterZ]) || PlayerToPoint(25.0,playerid,Businesses[bouse][ExitX],Businesses[bouse][ExitY],Businesses[bouse][ExitZ]))
				{
				    if(strfind( result , "|" , true ) == -1)
				    {
						...
						format(string, sizeof(string), "[INFO:} Zmieni³eœ nazwe na: %s.",Businesses[bouse][BusinessName]);
						SendClientMessage(playerid, COLOR_WHITE, string);
						SaveBusinesses();
					}
					else
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Z³y symbol/znak.");
					}
				}
				else
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Musisz byæ w swoim biznesie!");
				}
			}
			else
			{
 	...
 	if (strcmp(cmd, "/biznesinfo", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new bouse = PlayerInfo[playerid][pBizKey];
			...
			if(bouse != 255 && strcmp(playername, Businesses[PlayerInfo[playerid][pBizKey]][Owner], true) == 0)
			{
				...
				{
					format(string, sizeof(string), "[INFO:] Nazwa: %s - Sejf: $%d - Zamek(1-otwarty/0-zamkniêty): %d - Produkty: %d - Cena wejœcia: $%d.", Businesses[bouse][BusinessName],Businesses[bouse][Till],Businesses[bouse][Locked],Businesses[bouse][Products],Businesses[bouse][EntranceCost]);
					SendClientMessage(playerid, COLOR_WHITE, string);
					return 1;
				}
				else
  	...
  	if(strcmp(cmd, "/sprzedajbiznes", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
		{
			new playername[MAX_PLAYER_NAME];
			...
			if(PlayerInfo[playerid][pBizKey] != 255 && strcmp(playername, Businesses[PlayerInfo[playerid][pBizKey]][Owner], true) == 0)
			{
				...
				if(PlayerToPoint(1.0,playerid,Businesses[biz][EnterX],Businesses[biz][EnterY],Businesses[biz][EnterZ]))
				{
					...
					format(string, sizeof(string), "[INFO:] Biznes sprzedano za %d$.", Businesses[biz][BizPrice]);
					SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
					ChangeStreamPickupModel(Businesses[biz][PickupID],1272);
					PlayerInfo[playerid][pBizKey] = 255;
					OnPlayerDataSave(playerid);
  					PlayerActionMessage(playerid,15.0,"sprzedaje biznes agencji nieruchomoœci.");
					SaveBusinesses();
					return 1;
				}
				else
	...
	if(strcmp(cmd, "/kupbiznes", true) == 0)
	{
	    if(IsPlayerConnected(playerid))
	    {
			new Float:oldposx, Float:oldposy, Float:oldposz;
			...
			for(new h = 0; h < sizeof(Businesses); h++)
			{
				if(PlayerToPoint(2.0, playerid, Businesses[h][EnterX], Businesses[h][EnterY], Businesses[h][EnterZ]) && Businesses[h][Owned] == 0)
				{
				    ...
				    {
				        SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Ten biznes nie ma ustalonej ceny.");
						return 1;
				    }
					if(PlayerInfo[playerid][pBizKey] != 255 && strcmp(playername, Businesses[PlayerInfo[playerid][pBizKey]][Owner], true) == 0)
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Ju¿ masz jeden biznes, najpierw sprzedaj pierwszy!");
						return 1;
					}
					if(GetPlayerCash(playerid) > Businesses[h][BizPrice])
					{
						...
						SendClientMessage(playerid, COLOR_WHITE, "[INFO:] Kupiono biznes!");
	       				PlayerActionMessage(playerid,15.0,"podaje pieni¹dze i odbiera klucze do budynku.");
					...
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "[ERROR:] Nie masz tyle gotówki!");
						return 1;
					}
				}
			}
	...
	if(strcmp(cmd, "/prawojazdy", true) == 0)
	{
		if(PlayerToPoint(1.0,playerid,DrivingTestPosition[X],DrivingTestPosition[Y],DrivingTestPosition[Z]))
		{
		    if(PlayerInfo[playerid][pCarLic] == 0)
		    {
				...
				{
					GivePlayerCash(playerid,-500);
					SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[INFO:] Witamy na egzaminie! WejdŸ do auta na zewn¹trz budynku!");
			...
			{
				SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[ERROR:] Masz ju¿ prawojazdy!");
			}
		}
		else
		{
			SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"[ERROR:] Nie jesteœ w szkole jazdy!");
		}
		return 1;
	}
...
public OnPlayerRegister(playerid, password[])
{
	if(IsPlayerConnected(playerid))
	{
			...
			if (hFile)
			{
				...
				fclose(hFile);
				SendClientMessage(playerid, COLOR_YELLOW2, "[INFO:] Rejestracja przebieg³a pomyœlnie.");
